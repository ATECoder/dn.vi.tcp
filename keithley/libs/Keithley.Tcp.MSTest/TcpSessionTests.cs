...
public class TcpSessionTests
{
    ...
    public static void InitializeFixture( TestContext context )
    {
        try
        {
            ...
            Console.WriteLine( $"{nameof( TcpEchoServer )} is {_server.Running:'running';'limbo';'idle'}" );
    ...
    private static void OnServerPropertyChanged( object? sender, PropertyChangedEventArgs args )
    {
        switch ( args.PropertyName )
        {
            ...
            case nameof( TcpEchoServer.IPv4Address ):
                ...
                break;
            case nameof( TcpEchoServer.Running ):
                Console.WriteLine( $"{args.PropertyName} set to {_server?.Running}" );
                break;
        }
    }
